...
function BestsellerDetail() {
  ...
  const fetchProduct = async () => {
    try {
      const response = await fetch(`${API_URL}/bestseller/${id}`);
      ...
      toast.error('Failed to load product!', toastConfig);
      navigate('/');
    } finally {
      setLoading(false);
    }
    ...
    return (
      <div className="min-h-screen bg-black">
        ...
        <div className="flex flex-col items-center justify-center h-screen">
          ...
          <button onClick={() => navigate('/')} className="px-6 py-2 bg-white text-black rounded-lg">
            Back to Home
          </button>
        </div>
        <Footer />
