...
function AdminShopHero() {
  ...
  const fetchHero = async () => {
    try {
      const response = await fetch(`${API_URL}/shop-hero`);
      ...
      if (result.success && result.data) {
        ...
        setFormData({
          ...
          title: result.data.title || 'Our Collection',
          subtitle: result.data.subtitle || 'Discover premium craftsmanship and timeless design',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = hero ? `${API_URL}/shop-hero/${hero.id}` : `${API_URL}/shop-hero/1`;
      ...
      if (result.success) {
        ...
        setEditing(false);
        setImageFile(null);
        alert('Shop Hero saved successfully!');
      } else {
    ...
    <AdminLayout>
      <div className="max-w-4xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-2xl font-bold text-white">Shop Page Hero Section</h1>
          <button
            onClick={() => {
              setEditing(!editing);
              if (!editing && hero) {
          ...
          <div className="bg-gray-900 rounded-xl p-6">
            <div className="mb-6">
              <img 
                ...
                onError={(e) => {
                  e.target.src = 'https://via.placeholder.com/800x400?text=Shop+Hero';
                }}
              />
            </div>
          ...
          <form onSubmit={handleSubmit} className="bg-gray-900 rounded-xl p-6 space-y-4">
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full p-2 rounded bg-gray-800 text-white border border-white/10"
                required
                placeholder="e.g., Discover premium craftsmanship and timeless design"
              />
            </div>
