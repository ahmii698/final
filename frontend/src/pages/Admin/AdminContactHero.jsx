...
function AdminContactHero() {
  ...
  const fetchHero = async () => {
    try {
      const response = await fetch(`${API_URL}/contact-hero`);
      ...
      if (result.success && result.data) {
        ...
        setFormData({
          ...
          title: result.data.title || 'Get In Touch',
          subtitle: result.data.subtitle || "We'd love to hear from you",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = hero ? `${API_URL}/contact-hero/${hero.id}` : `${API_URL}/contact-hero/1`;
      ...
      if (result.success) {
        ...
        setEditing(false);
        setImageFile(null);
        alert('Contact Hero saved successfully!');
      } else {
    ...
    <AdminLayout>
      <div className="max-w-4xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-2xl font-bold text-white">Contact Page Hero Section</h1>
          <button
            onClick={() => {
              setEditing(!editing);
              if (!editing && hero) {
          ...
          <div className="bg-gray-900 rounded-xl p-6">
            <div className="mb-6">
              <img 
                ...
                onError={(e) => {
                  e.target.src = 'https://via.placeholder.com/800x400?text=Contact+Hero';
                }}
              />
            </div>
