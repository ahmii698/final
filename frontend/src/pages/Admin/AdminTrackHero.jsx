...
function AdminTrackHero() {
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        setFormData({
          ...
          title: result.data.title || 'Track Your Order',
          subtitle: result.data.subtitle || 'Stay updated on your delivery status',
        ...
        setFormData({
          ...
          subtitle: 'Stay updated on your delivery status',
          active: 1
        });
      }
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = hero ? `${API_URL}/track-hero/${hero.id}` : `${API_URL}/track-hero/1`;
      ...
      if (result.success) {
        ...
        setEditing(false);
        setImageFile(null);
        alert('Track Order Hero saved successfully!');
      } else {
    ...
    <AdminLayout>
      <div className="max-w-4xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-2xl font-bold text-white">Track Order Page Hero Section</h1>
          <button
            onClick={() => {
              setEditing(!editing);
              if (!editing && hero) {
          ...
          <div className="bg-gray-900 rounded-xl p-6">
            <div className="mb-6">
              <img 
                ...
                onError={(e) => {
                  e.target.src = 'https://via.placeholder.com/800x400?text=Track+Order+Hero';
                }}
              />
            </div>
          ...
          <form onSubmit={handleSubmit} className="bg-gray-900 rounded-xl p-6 space-y-4">
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full p-2 rounded bg-gray-800 text-white border border-white/10"
                required
                placeholder="e.g., Stay updated on your delivery status"
              />
            </div>
