...
function AdminBestsellers() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = editingProduct 
        ? `${API_URL}/bestsellers/${editingProduct.id}`
        : `${API_URL}/bestsellers`;
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this product?')) {
      try {
        const response = await fetch(`${API_URL}/bestsellers/${id}`, {
          method: 'DELETE'
        });
        const result = await response.json();
        if (result.success) {
  ...
  return (
    <AdminLayout>
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-white">Bestseller Products</h1>
        <button
          onClick={() => {
            resetForm();
            setShowModal(true);
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 overflow-y-auto">
          <div className="bg-gray-900 rounded-xl p-6 w-full max-w-md max-h-[90vh] overflow-y-auto">
            <h2 className="text-xl font-bold text-white mb-4">{editingProduct ? 'Edit' : 'Add'} Bestseller Product</h2>
